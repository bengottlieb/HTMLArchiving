...
- (NSString * __nullable) metaContentForProperty: (NSString *) property {
	NSArray			*metas = [self nodesMatchingSelector: @"meta"];
	
	for (HTMLElement *meta in metas) {
		NSString		*prop = meta.attributes[@"property"];
		
		if ([prop isEqual: property]) { return meta.attributes[@"content"]; }
	}
	return nil;
}
...
- (HTMLArrayOf(HTMLElement *) *) nodesWithStyleTag: (NSString *) tag
{
	NSMutableArray *ret = [NSMutableArray new];
	for (HTMLElement *node in self.treeEnumerator) {
		if ([node isKindOfClass:[HTMLElement class]] && node.attributes[@"style"] != nil && [node.attributes[@"style"] containsString: tag]) {
			NSDictionary			*style = node.parsedStyleTag;
